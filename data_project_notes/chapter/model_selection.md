...
1. 偏差定义为：$bias(\hat{\theta}_m)=E(\hat{\theta}_m)-\theta$，期望作用在所有数据上。
    ...
    * 如果$\lim_{m \to \infty} bias(\hat{\theta}_m)=$，则称估计量$\hat{\theta}_m$是渐近无偏的。
...
# 四、参数估计准则
## 1. 最大似然估计
1. 假设数据集$X=\lbrace {\vec x}_1,{\vec x}_2,...,{\vec x}_m \rbrace$中的样本独立同分布地由$p_{data}(\vec x)$产生，但是该分布是未知的。$p_{model}(\vec x;\theta)$是一组由$\theta$参数控制的概率分布函数族，希望通过$p_{model}(\vec x;\theta)$来估计真实的概率分布函数$p_{data}(\vec x)$，也就是要估计$\theta$参数。

2. 最大似然估计最大化数据集$X$出现的概率。即：
$$
\theta_{ML} = \mathop{\arg max}_{\theta}\; p_{model}(X;\theta)
= \mathop{\arg max}_{\theta} \prod_{i=1}^{m} p_{model}({\vec x}_i;\theta)
$$
    * 由于概率的乘积会因为很多原因不便使用（如容易出现数值下溢出），因此转换为对数的形式：
$$\theta_{ML}= \mathop{arg max}_{\theta}\sum_{i=1}^{m} log\;p_{model}(\vec x;\theta)$$
    * 因为m与$\theta$无关，因此它也等价于：
$$\theta_{ML}= \mathop{arg max}_{\theta}\sum_{i=1}^{m} \frac{1}{m}log\;p_{model}(\vec x;\theta)$$
    * 由于数据集的经验分布为：
$${\hat p}_{data}(\vec x)=\frac{1}{m}\sum_{1}^{m}\delta({\vec x}-{\vec x}_i)
$$其中$\delta(\cdot)$为狄拉克函数。因此：
$$\theta_{ML}= \mathop{arg max}_{\theta}E_{{\vec x}~{\hat p}_{data}}log\;p_{model}(\vec x;\theta)$$

3. 考虑数据集的经验分布${\hat p}_{data}$和真实分布函数的估计量$p_{model}$之间的差异，KL散度为：
...
4. 最大后验估计估计`MAP`提供了一个直观的方法去设计复杂的、可解释的正则化项。更复杂的正则化项可以通过先验分布为混合高斯分布得到（而不仅仅是一个单独的高斯分布）。
...
![](https://upload-images.jianshu.io/upload_images/16911112-a3ad6f31ab72c130.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
